...
CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 python finetune.py \
    ...
    --output_dir checkpoints/finetune-pretrain-1024-gpt-noprior/instructor-large-${dataset}-d=${d}-epoch=${epoch}-iter=$1 \
    --train_file converted_triplet_results/${dataset}_embed=instructor_s=${scale}_m=1024_d=${d}_sf_choice_seed=${seed}-mistral_7b-train=$(($1-1)).json \
    --cache_dir cache \
...
#     --train_file converted_triplet_results/${dataset}_embed=instructor_s=${scale}_m=1024_d=${d}_sf_choice_seed=${seed}-mistral_7b-self-train.json \
#     --cache_dir cache \
#     --max_source_length 512 \
#     --num_train_epochs $epoch \
#     --per_device_train_batch_size 4 \
